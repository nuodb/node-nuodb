...
class Pool {
  ...
  async _checkAllAndRemove(_id) {
    try {
      await this.all_connections[_id].connection._close();
    } catch (e) {
  ...
  async closePool() {
    ...
    await Promise.all(
      Object.keys(this.all_connections).map(async (key) => {
        try {
          await this.all_connections[key].connection._close();
...
//! make sure that closing connection works with change
//! add actual nuoDB connection ID to the connection
//! give possible arg to pool to give it an ID
