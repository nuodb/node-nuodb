...
class Pool {
  constructor(args) {
    this.config = {
      ...
      // hard limit not implemented yet, set to prevent extreme cases. Possible setting of 0 for no hard limit (use at own risk :evil face:)
    };
    ...
    this.init();
    setInterval(this.livelinessCheck, this.config.checkTime);
  }
  ...
  async livelinessCheck() {
    ...
    while (checked[toCheck.id] != true && this.free_connections.length() > 0) {
      ...
      if (connectionAlive) {
        ...
        let newConn = await this.connectionAgeOut(toCheck.id); // need to check if you actually get connection
        this.free_connections.push(newConn);
      }
    }
  }
  // may want better name for this function, closeConnection
  async connectionAgeOut(id) {
        ...
        for (let j = 0; j < this.all_connections.length(); j++) {
          if (this.all_connections[j].id === id) {
            ...
            if (this.connection_limit > this.all_connections.length()) {
              newConn = await this.createConnection();
              this.free_connections.push(newConn);
              return newConn;
            } else {
              return;
  ...
  async createConnection() {
    if (this.config.hardLimit > 0) {
      if (this.all_connections.length() >= this.config.hardLimit) {
        return "connection hard limit reached"; //not right
      }
    }
    let connection;
    const driver = new Driver();
    try {
      // todo put this black into a new function _createConnection _ delineates functions NOT for users
      ...
      setTimeout(this.connectionAgeOut, this.config.max_age, id);
      return connection;
    } catch (err) {
  ...
  // TODO change name "releaseConnection", ensure that connections get released to correct pool
  // could add method to connection that targets a pool, or just have reference to pool
  async connectionReturn(connection) {
    ...
    const connectionAlive = await this.checkConnection(connection); //returns boolean
    if (connectionAlive) {
      this.free_connections.push(connection);
    } else {
      const newConn = await this.connectionAgeOut(connection.id);
      this.free_connections.push(newConn);
    }
  }
}
...
timers possible add state variable init, running, closed. look into better comments */
